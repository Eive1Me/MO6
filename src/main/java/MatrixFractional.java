...
public class MatrixFractional extends Matrix<Double> {
    ...
    public void countMinAndMax(){
        int height = this.height();
        int width = this.width();
        for (String columnName : this.columnNames()) {
            Cell cell = new Cell(columnName, "y" + (height + 1));
            this.set(cell, this.maxInColumnValue(columnName, new ArrayList<>()));
        }
        for (String rowName : this.rowNames()) {
            Cell cell = new Cell("x" + (width + 1), rowName);
            if (rowName.equals("y" + (height + 1))) continue;
            this.set(cell, this.minInRowValue(rowName, new ArrayList<>()));
        }
    }
    ...
    public String toStringShort() {
        ...
        for (String rowName : rowNames) {
            String firstColumnSpace = String.join("", Collections.nCopies(cellLength - rowName.length(), " "));
            if (Integer.parseInt(rowName) < 10 || (rowNames.size() - Integer.parseInt(rowName) < 5)) {
            answer += " " + firstColumnSpace + rowName + " ";
                for (String columnName : columnNames) {
                    Cell cell = new Cell(columnName, rowName);
                    String valueAsString = "";
                ...
                answer += "\n";
            } else if ((rowNames.size() - Integer.parseInt(rowName)) == 5)
                answer += "  " + firstColumnSpace + "...\n";
        }
        answer += horizontalLine;
        return answer;
    }
